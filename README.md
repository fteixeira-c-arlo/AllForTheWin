...
**E3 Wired (VMC2070, VMC3070, VMC2083, VMC3083):** The available command list is loaded from `commands/e3_wired_commands.json`, which was populated from Arlo Confluence via arlochat MCP. Source page: [Arlo E3 Wired - How to use CLI on Console](https://arlo.atlassian.net/wiki/spaces/AFS/pages/63449494/Arlo+E3+Wired+-+How+to+use+CLI+on+Console). When you connect an E3 Wired camera (ADB or SSH), the terminal shows these CLI commands (e.g. `build_info`, `reboot`, `migrate`, `ptz`, `caliget`/`caliset`). To refresh the list from Confluence, use arlochat MCP in Cursor with `confluence_search(cql='title ~ "E3 Wired" AND title ~ "CLI"')` and update `e3_wired_commands.json` if needed.
